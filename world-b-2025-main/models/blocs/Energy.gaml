...
/* Setup */
	...
	factory_season_factor <- ["nuclear"::["spring"::0.25, "summer"::0.25, "autumn"::0.25, "winter"::0.25], "hydro"::["spring"::0.38, "summer"::0.18, "autumn"::0.28, "winter"::0.16], "wind"::["spring"::0.22, "summer"::0.12, "autumn"::0.27, "winter"::0.39], "solar"::["spring"::0.26, "summer"::0.48, "autumn"::0.18, "winter"::0.08]];

	// Production par infrastructures et par tick (kWh)
	map<string, map<string, float>>
	factory_production <- ["spring"::["nuclear"::(total_factory_production["nuclear"] * factory_season_factor["nuclear"]["spring"]) / 3, "hydro"::(total_factory_production["hydro"] * factory_season_factor["hydro"]["spring"]) / 3, "wind"::(total_factory_production["wind"] * factory_season_factor["wind"]["spring"]) / 3, "solar"::(total_factory_production["solar"] * factory_season_factor["solar"]["spring"]) / 3], "summer"::["nuclear"::(total_factory_production["nuclear"] * factory_season_factor["nuclear"]["summer"]) / 3, "hydro"::(total_factory_production["hydro"] * factory_season_factor["hydro"]["summer"]) / 3, "wind"::(total_factory_production["wind"] * factory_season_factor["wind"]["summer"]) / 3, "solar"::(total_factory_production["solar"] * factory_season_factor["solar"]["summer"]) / 3], "autumn"::["nuclear"::(total_factory_production["nuclear"] * factory_season_factor["nuclear"]["autumn"]) / 3, "hydro"::(total_factory_production["hydro"] * factory_season_factor["hydro"]["autumn"]) / 3, "wind"::(total_factory_production["wind"] * factory_season_factor["wind"]["autumn"]) / 3, "solar"::(total_factory_production["solar"] * factory_season_factor["solar"]["autumn"]) / 3], "winter"::["nuclear"::(total_factory_production["nuclear"] * factory_season_factor["nuclear"]["winter"]) / 3, "hydro"::(total_factory_production["hydro"] * factory_season_factor["hydro"]["winter"]) / 3, "wind"::(total_factory_production["wind"] * factory_season_factor["wind"]["winter"]) / 3, "solar"::(total_factory_production["solar"] * factory_season_factor["solar"]["winter"]) / 3]];
	...
	/*
		...
		*/
	"m² land"::["nuclear"::1500000.0, "wind"::500.0, "hydro"::2000000.0, "solar"::20000.0], "L water"::["nuclear"::800000.0, "wind"::50.0, "hydro"::0.0, "solar"::100.0]
	/**/];

	//Émissions par kWh (gCO2e)
	map<string, float> emissions_per_kWh <- ["nuclear"::4.0, "hydro"::6.0, "wind"::15.0, "solar"::34.0];

	// Coût de construction (ressources nécessaires par infrastructure)
	map<string, map<string, float>>
	factory_construction_cost <- ["m3_wood"::["nuclear"::12000.0, "wind"::45.0, "hydro"::8500.0, "solar"::5.0], "kg_cotton"::["nuclear"::500000.0, "wind"::1200.0, "hydro"::350000.0, "solar"::150.0], "m² land"::["nuclear"::1500000.0, "wind"::500.0, "hydro"::2000000.0, "solar"::20000.0]];
	...
	// Utilisation de l'énergie par les autres secteur
	map<string, float> used_energy <- [];
...
species infrastructure {
	...
	string current_season {
		...
		if (saison_index < 3) {
			return "spring";
		} else if (saison_index < 6) {
			return "summer";
		} else if (saison_index < 9) {
			return "autumn";
		} else {
			return "winter";
		} }
...
species energy parent: bloc {
	...
	action setup {
		...
		// TODO : gérer la pénurie
			float land_ok <- producer.ext_producers["m² land"].producer.produce(self.name, ["m² land"::total_land_needed]);
		}
	...
	action build_infrastructure (string energy_type, int quantity) {
		loop mat over: ["m3_wood", "kg_cotton", "m² land"] {
			...
			if (not (producer.ext_producers contains_key mat) or not (producer.ext_producers[mat].producer.produce(self.name, [mat::cost]))) {
				return;
			}
	...
	action remove_oldest_factory (string energy_type) {
		...
		if (energy_type = "nuclear") {
			candidates <- reacteur select (each.operational);
		} else if (energy_type = "hydro") {
			candidates <- centrale_hydro select (each.operational);
		} else if (energy_type = "wind") {
			candidates <- parc_eolien select (each.operational);
		} else if (energy_type = "solar") {
			candidates <- champ_solaire select (each.operational);
		}
	...
	bool re_use_factory (string energy_type) {
		...
		if (energy_type = "nuclear") {
			unused <- reacteur select (not each.operational);
		} else if (energy_type = "hydro") {
			unused <- centrale_hydro select (not each.operational);
		} else if (energy_type = "wind") {
			unused <- parc_eolien select (not each.operational);
		} else if (energy_type = "solar") {
			unused <- champ_solaire select (not each.operational);
		}
	...
	action optimize_park {
		...
		map<string, int> min_capacity <- ["nuclear"::1, "hydro"::1, "wind"::1, "solar"::1];

		//Nombre d'usines modifié max
		map<string, int> batch_size <- ["nuclear"::1, "hydro"::26, "wind"::30, "solar"::13];
		...
		//Trouver le type avec le plus grand écart positif
			...
			loop energy_type over: energy_types {
				...
				map<string, int> operational_count <- ["nuclear"::length(reacteur select (each.operational)), "hydro"::length(centrale_hydro select
				(each.operational)), "wind"::length(parc_eolien select (each.operational)), "solar"::length(champ_solaire select (each.operational))];
...
species energy_producer parent: production_agent {
	...
	action reset_tick_counters {
		...
		// Reset des demandes des autres secteurs
		used_energy <- [];
	}
	...
	bool produce (string buyer, map<string, float> demand) {
		bool ok <- true;
		//		write "(" + self.name + ") - demandeur = " + buyer + ", ressource = " + demand;
		loop product over: demand.keys {

		// Récupérer les demandes d'énergies des différents secteur
			if (product = "kWh energy") {
				tick_external_demand_E <- tick_external_demand_E + demand[product];
				if (buyer contains "farm") {
					used_energy["farms"] <- used_energy["farms"] + demand[product];
				} else if (buyer contains "transport") {
					used_energy["transport"] <- used_energy["transport"] + demand[product];
				} else {
					used_energy[buyer] <- demand[product];
				}
	...
	action stock_update {
		...
		bool av <- ext_producers["L water"].producer.produce(self.name, ["L water"::ress["L water"]]);
		if (av) {
			ask energy {
...
species energy_consumer parent: consumption_agent {
	...
	action consume (human h) {
		...
		consumed["kWh energy"] <- consumed["kWh energy"] + rnd(min_kWh_conso[saison], max_kWh_conso[saison]);
	}
...
experiment run_energy type: gui {
	output {
		...
		display Energy_usage type: 2d {
			chart "Utilisation de l'énergie pour chaque secteur (en kWh)" type: pie {
				data "Fermes" value: used_energy["farms"] color: #brown;
				data "Transports" value: used_energy["transport"] color: #blue;
			}

		}

		monitor "Secteur qui demande de l'énergie : " value: used_energy;
	}
