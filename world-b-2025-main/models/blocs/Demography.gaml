	...
	action tick (list<human> pop) {
		...
		// Besoin total en viande
			...
			// Capture les besoins en viande
				...
				int school_agents <- length(self.individuals where (each.age >= 6 and each.age <= 18));
				int work_agents <- length(self.individuals where (each.age >= 18 and each.age <= 67));
				
				//self.go_to_school <- float(max([1, school_agents]) * self.factor_individuals);
				self.go_to_school <- float(school_agents * self.factor_individuals);
				//self.go_to_work <- float(max([1, work_agents]) * self.factor_individuals);
				self.go_to_work <- float(work_agents * self.factor_individuals); 
