...
species transport parent: bloc {
	...
	species short_trip {
		map<string, float> short_trip_decisions <- ["trip_bike"::0.05, "trip_minibus"::0.3, "trip_walking"::0.65];
		...
		action process_short_trips (int trip_number) {
			...
			loop mode over: short_trip_decisions.keys {
				if (mode != "trip_walking") {
					...
					tick_trips_by_mode[mode] <- trip_number * short_trip_decisions[mode] + next_mode;
				}
...
//        }
	...
	output {
		display Transport_information type: 2d{
			...
			/*chart "Number of long trips by mode" type: series size: {0.5,0.5} position: {0.5, 0.5} {
...
//	    		}
			...
			chart "Transport usage (%)" type: series size: {0.5,0.5} position: {0.5,0.5} {
			    data "trip_minibus" value: transport_usage["trip_minibus"] * 100 color: #red;
			    data "trip_tgv"     value: transport_usage["trip_tgv"] * 100     color: #blue;
			    data "trip_ter"     value: transport_usage["trip_ter"] * 100     color: #green;
			    data "trip_taxi"    value: transport_usage["trip_taxi"] * 100    color: #yellow;
			    data "trip_bike"    value: transport_usage["trip_bike"] * 100    color: #purple; 
			}
