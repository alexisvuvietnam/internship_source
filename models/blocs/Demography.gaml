	...
	action tick (list<human> pop) {
		...
		// Besoin total en viande
			...
			// Capture les besoins en viande
				...
				// Calcul du nombre d'élèves par convention
				go_to_school <- length(self.individuals where (each.age >= 6 and each.age <= 18));
				
				// Calcul du nombre de travailleurs par convention
				go_to_work <- length(self.individuals where (each.age >= 18 and each.age <= 67)); 
